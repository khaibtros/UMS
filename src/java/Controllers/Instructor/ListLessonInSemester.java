 ...
 * @author User
 */
public class ListLessonInSemester extends HttpServlet {
    ...
    @Override
     protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (instructor == null) {
            ...
            response.sendRedirect("/ums/login"); // Chuyển hướng về trang login nếu cần
            return;
        }

        int instructorId = instructor.getInstructorID(); // Lấy InstructorID

        // Lấy semesterId từ query string hoặc request parameter
        String semesterIdParam = request.getParameter("semesterId");
        if (semesterIdParam == null || semesterIdParam.isEmpty()) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing or invalid semesterId.");
            return;
        }
        ...
        try {
            ...
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid semesterId format.");
            return;
        }

        // Gọi DAO để lấy danh sách lesson giảng viên dạy trong kỳ học này
        LessonDAO lessonDAO = new LessonDAO();
        List<Lesson> lessons = lessonDAO.getLessonsByInstructorAndSemester(instructorId, semesterId);
